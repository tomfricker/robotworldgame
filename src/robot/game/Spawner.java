...
public class Spawner {
	...
	public void tick() {
		...
		switch(hud.getLevel()) {
			...
			case 1: LevelOneStage1();
			
			//level 2
			case 2: LevelTwoStage1();
	 ...
	 */
	public void LevelOneStage1() {
		if(hud.getScore() == 15) {
			manager.addObject(new LevelTomNPC2(Cells.A, Cells.G, ID.NPC, manager, side, hud));
			hud.setScore(hud.getScore() + 5);
		}
		else if(hud.getScore() == 25) {
			manager.addObject(new LevelTomNPC3(Cells.H, Cells.G, ID.NPC, manager, side, hud));
			hud.setScore(hud.getScore() + 5);
		}
